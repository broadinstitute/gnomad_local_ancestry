...
def subset_and_export_chr_vcfs(mt: hl.MatrixTable, samples_path: str, output_bucket: str, contigs: list, sparse: bool = True, gt_expr: str = "LGT") -> hl.MatrixTable:
    """
    ...
    for contig in contigs:
        contig = f"chr{contig}"
        mt = hl.filter_intervals(mt, [hl.parse_locus_interval(contig, reference_genome="GRCh38")])
        mt = subset_samples_and_variants(mt, samples_path, sparse=sparse, gt_expr=gt_expr)

        if sparse:
            mt = hl.experimental.sparse_split_multi(mt, filter_changed_loci=True)
            mt = hl.experimental.densify(mt)
            mt = mt.filter_rows(hl.len(mt.alleles) > 1).drop('gvcf_info') # Note: This step is sparse-specific, removing monoallelic sites after densifying
        else:
        ...
        mt = mt.checkpoint(f"{output_bucket}{contig}/gnomad_{contig}_dense_bia_snps.mt", overwrite=True)
        hl.export_vcf(mt, f"{output_bucket}{contig}/gnomad_{contig}_dense_bia_snps.vcf.bgz")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--contigs', nargs='?', help='comma separated integer contigs to run')
    args = parser.parse_args()
    subset_and_export_chr_vcfs(args.mt, args.samples_path, args.output_bucket, args.contigs, args.sparse, args.gt_expr)
